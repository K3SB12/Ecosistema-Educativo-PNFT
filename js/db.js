...
export async function openDB() {
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            const stores = [
                'plannings', 'profile', 'groups', 'periods', 'evaluation_indicators',
                'centros_educativos', 'school_years', 'periods_enhanced', 'components',
                'settings', 'students', 'attendance', 'grades', 'subjects', 'schedules',
                'daily_work_indicators', 'evaluations', 'bitacoras', 'conducta', 'alertas',
                'accommodation_types'
            ];
...
export async function initDefaultData() {
    ...
    if (centros.length === 0) {
        await put('centros_educativos', { id: Date.now(), nombre: 'Escuela Tranquilino Sáenz Rojas', codigo: 'CE1', activo: true });
    }
    ...
    if (periodos.length === 0) {
        ...
        if (year2026) {
            await put('periods_enhanced', { id: Date.now(), schoolYearId: year2026.id, nombre: 'I Semestre', tipo: 'semestre', fechaInicio: '2026-02-23', fechaFin: '2026-07-03', activo: true, porcentajeAnual: 50 });
            await put('periods_enhanced', { id: Date.now() + 1, schoolYearId: year2026.id, nombre: 'II Semestre', tipo: 'semestre', fechaInicio: '2026-07-20', fechaFin: '2026-12-09', activo: true, porcentajeAnual: 50 });
        }
    }
    ...
    if (componentes.length === 0) {
        const standardComponents = [
            { id: Date.now(), nombre: 'I Ciclo', subcomponentes: [{ nombre: 'Trabajo cotidiano', porcentaje: 65 }, { nombre: 'Tareas', porcentaje: 10 }, { nombre: 'Prueba de ejecución', porcentaje: 15 }, { nombre: 'Asistencia', porcentaje: 10 }] },
            { id: Date.now() + 1, nombre: 'II Ciclo', subcomponentes: [{ nombre: 'Trabajo cotidiano', porcentaje: 60 }, { nombre: 'Tareas', porcentaje: 10 }, { nombre: 'Prueba de ejecución', porcentaje: 20 }, { nombre: 'Asistencia', porcentaje: 10 }] },
            { id: Date.now() + 2, nombre: 'III Ciclo', subcomponentes: [{ nombre: 'Trabajo cotidiano', porcentaje: 50 }, { nombre: 'Tareas', porcentaje: 10 }, { nombre: 'Proyecto', porcentaje: 30 }, { nombre: 'Asistencia', porcentaje: 10 }, { nombre: 'Portafolio', porcentaje: 0 }] },
            { id: Date.now() + 3, nombre: 'Diversificada', subcomponentes: [{ nombre: 'Trabajo cotidiano', porcentaje: 50 }, { nombre: 'Tareas', porcentaje: 5 }, { nombre: 'Proyecto', porcentaje: 35 }, { nombre: 'Asistencia', porcentaje: 10 }, { nombre: 'Portafolio', porcentaje: 0 }] },
            { id: Date.now() + 4, nombre: 'Educación Adultos', subcomponentes: [{ nombre: 'Trabajo cotidiano', porcentaje: 50 }, { nombre: 'Tareas', porcentaje: 10 }, { nombre: 'Proyecto', porcentaje: 30 }, { nombre: 'Asistencia', porcentaje: 10 }, { nombre: 'Portafolio', porcentaje: 0 }] },
            { id: Date.now() + 5, nombre: 'Educación Especial', subcomponentes: [{ nombre: 'Trabajo cotidiano', porcentaje: 60 }, { nombre: 'Tareas', porcentaje: 10 }, { nombre: 'Proyecto', porcentaje: 20 }, { nombre: 'Asistencia', porcentaje: 10 }, { nombre: 'Portafolio', porcentaje: 0 }] }
        ];
        for (let comp of standardComponents) await put('components', comp);
    }
    ...
    if (accommodationTypes.length === 0) {
        ...
        for (let type of defaultTypes) await put('accommodation_types', type);
    }
}
...
export async function calculateAnnualGrade(studentId, groupId, subjectId, decimals = 2) {
    ...
    for (let period of yearPeriods) {
        ...
        if (peso === 0) peso = 100 / yearPeriods.length;
        if (peso === 0) continue;
